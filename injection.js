...
(function() {
	...
	addReplacement('VERSION$1," | ",', `"${vapeName} v1.0.0"," | ",`);
	...
	addReplacement('async loadSpritesheet(){', `
		async loadVape() {
			this.vapeTexture = await this.loader.loadAsync("https://raw.githubusercontent.com/catInfinite/VapeVhForMiniblox/main/assets/logo.png");
			this.vhTexture = await this.loader.loadAsync("https://raw.githubusercontent.com/catInfinite/VapeVhForMiniblox/main/assets/logovh.png");
		}
	...
	addReplacement('(this.drawSelectedItemStack(),this.drawHintBox())', `
		if (ctx$3 && enabledModules["TextGUI"]) {
			...
			drawImage(ctx$3, textureManager.vhTexture.image, posX + 81, posY + 1, 33, 18);
	...
	addReplacement('document.addEventListener("contextmenu",j=>j.preventDefault());', `
		...
		(function() {
			...
			speedvalue = speed.addoption("Speed", Number, 2.0);
			speedjump = speed.addoption("JumpHeight", Number, 0.35);
